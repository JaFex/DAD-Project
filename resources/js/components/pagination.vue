<template>
    <nav>
        <ul class="pagination justify-content-center">
            ...
            <template v-if="links.currentPage >= 4" v-for="i in links.lastPage">
                <li v-if="links.currentPage-2 <= i && i <= links.currentPage+2" class="page-item"  :class="{'active' : links.currentPage == i}">
                    <a class="page-link" href="#" @click.prevent="loadMethod(links.path+i)">{{i}}</a>
                </li>
            </template>
            <template v-if="links.currentPage < 4" v-for="i in links.lastPage">
                <li v-if="i < 6" class="page-item" :class="{'active' : links.currentPage == i}">
                    <a class="page-link" href="#" @click.prevent="loadMethod(links.path+i)">{{i}}</a>
                </li>
            </template>
