        ...
        <div class="input-group mb-3">
          ...
          <div class="input-group-append">
            <button
              ...
              type="button"
              @click.prevent="creatMeal(selectedRestaurantTables)"
            >Create a meal for table {{ selectedRestaurantTables }}</button>
          </div>
        </div>
      ...
      <div v-if="showCreteOrder" class="card">
        <div class="card-header">
          ...
          <button
            ...
            @click.prevent="showCreteOrder = false"
          >Close</button>
        </div>
        <div class="card-body">
          <createOrder
            @clickTimeRunOutOrderConfirmed="timeRunOutOrderConfirmed"
            @clickReloadMealAndOrder="reloadMealAndOrder"
            :myMeals="meals"
            :meal="currentMealCreate"
          ></createOrder>
        </div>
      </div>
      <div class="card">
            ...
            <tbody>
              <template v-for="meal in meals">
                <tr :key="meal.id">
                  ...
                  <td>
                    <button
                      v-if="meal.total_price_preview > 0"
                      type="button"
                    ...
                    <button
                      type="button"
                      class="btn btn-primary"
                      @click.prevent="showFormCreteOrder(meal)"
                    >add order</button>
                    <button
                      type="button"
                      class="btn btn-danger"
                      @click.prevent="terminateMeal(meal)"
                    >Terminate</button>
                  </td>
                </tr>
...
export default {
  ...
  methods: {
      ...
      axios
        ...
        .catch(function(error) {
          ...
          if (error.response.data.errors.table_number) {
            console.log(
              "creatMeal-" + error.response.data.errors.table_number[0]
            );
        ...
        axios
          ...
          .catch(function(error) {
            ...
            if (error.response.status === 404) {
              console.log(
                "timeRunOutOrderConfirmed->" +
                  "Is 404 maybe order has been deleted so he didn t find the order"
              );
            } else {
    ...
    terminateMeal: function(meal) {
      let soft = this;
      axios
        .get("/api/meals/" + meal.id + "/terminated")
        .then(response => {
          ...
          if (booleanCanBeTerminated == true) {
            ...
            if (
              confirm(
                "This meal's (" +
                  meal.id +
                  ") orders have not been delivered yet, and they will be excluded from the payment! Is this okay?"
              )
            ) {
              soft.sendUpdateToTerminateMeal(meal);
            } else {
              soft.$toasted.show(
                "The meal (" + meal.id + ") has not been terminated",
                {
                  theme: "bubble",
    ...
    sendUpdateToTerminateMeal: function(meal) {
      ...
      axios
        .put("/api/meals/" + meal.id + "/terminated", update)
        .then(response => {
          ...
          soft.currentMeal =
            soft.currentMeal.id == meal.id ? null : soft.currentMeal;
        ...
        .catch(function(error) {
          ...
          soft.$toasted.show(
            "ERRO: The meal (" + meal.id + ") has not been terminated",
            {
              theme: "bubble",
              position: "bottom-center",
              duration: 5000,
